...
- [x] **Optimistic catch + payload shrink (PR #32, 2026-08-06):** cutout cropped to bbox (+5% margin) and downscaled to ≤1024px longest edge before upload (multi-MB → sub-1MB target, `sticker_shrinker.dart` via package:image in a compute isolate); catch result screen shows INSTANTLY with on-device sticker + "Naming your cat…", save runs in background with 2 auto-retries (2s/4s backoff) then gentle error + manual Retry — catch never lost. Codemagic→Discord notifications verified live (app-level var groups break team-group resolution — webhook URL stored inline in codemagic.yaml, notify step can never fail a build). TestFlight build 6a7451e02e19446706732e11 triggered.
