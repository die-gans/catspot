...
- [ ] **Sprint 1 STARTED 2026-07-29:** ✅ design system v1 (PR #1), ✅ scaffold S1.1 (PR #2), ✅ CI S1.2 (PR #3, green on main), ✅ S1.4 theme tokens (PR #4, CI-gated). ✅ V1/V2 convex + firebase auth wiring (PR #6). Firebase Auth swap COMPLETE 2026-07-30 (BE PR #7 + MOB PR #6, dev deployment on by_firebaseUid, project catspot-9ee0d). ✅ Bundle IDs aligned app.catspot.mobile (PR #8) + FlutterFire configure (PR #9, real firebase_options.dart, apps registered on catspot-9ee0d). ✅ S1.6 Codemagic pipelines (PR #10: android debug smoke runnable today; .fvmrc pinned 3.44.8). ✅ Apple Developer Program ACTIVE 2026-07-31 — ios-dev-ipa workflow unblocked. ✅ App ID capabilities (Sign in with Apple, Push) + device + app record registered 2026-07-31. ✅ MOB iOS runner prep (PR #11: camera plist string, Sign in with Apple + aps-development entitlements, pbxproj wired). ✅ Codemagic fixes: yaml moved to repo root (PR #12), FVM install step + $HOME/fvm cache (PR #13, fixes exit 127). Codemagic API token working; yaml workflows triggerable via API despite UI mode. Android smoke build attempt #2 in flight. Follow-up card needed: sign_in_with_apple + firebase_messaging Dart packages. Codemagic API token held by ORC (secrets/codemagic.env). ✅ ios-dev-ipa GREEN 2026-08-03 (build 6a7045905c965b870ca344d2 — signed dev IPA produced; Dan fixed remaining build issue manually). Next: ios-testflight workflow ✅ MERGED 2026-08-04 (PR #15: app_store signing via catspot_asc, submit_to_testflight). TestFlight build #1 FAILED preflight (no app_store profile). ORC created Apple Distribution cert (Y64Q7FZRD2) + App Store profile "Catspot AppStore" (B34KP5FBXQ) via ASC API 2026-08-04; p12+profile sent to Dan for Codemagic identity upload, then retrigger. ✅ ios-testflight build #2 GREEN 2026-08-04 (build 6a71331c3350c6384f24090f, commit ae89d280 — all steps success incl. publishing, ~8min; IPA submitted to App Store Connect). Dan TODO: answer export compliance in TestFlight + add himself as internal tester. PR-review bot: research done (feasible, cron-polling profile), verdict BUILD — Dan said NO-GO for now
